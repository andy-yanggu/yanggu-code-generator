            ...
            <#list formList as field>
                <el-form-item label="${field.fieldComment!}" prop="${field.attrName}">
                    <#if field.formType == 'text'>
                        <el-input v-model="state.dataForm.${field.attrName}" clearable
                                  placeholder="请输入${field.fieldComment!}"></el-input>
                    <#elseif field.formType == 'textarea'>
                        <el-input type="textarea" v-model="state.dataForm.${field.attrName}"></el-input>
                    <#elseif field.formType == 'editor'>
                        <el-input type="textarea" v-model="state.dataForm.${field.attrName}"></el-input>
                    <#elseif field.formType == 'select'>
                        <el-select v-model="state.dataForm.${field.attrName}" clearable filterable
                                   placeholder="请选择${field.fieldComment!}">
                            <el-option v-for="item in ${field.enumNameAllUpper}_ENUM" :key="item.value"
                                       :label="item.label" :value="item.value"></el-option>
                        </el-select>
                    <#elseif field.formType == 'radio'>
                        <el-radio-group v-model="state.dataForm.${field.attrName}">
                            <el-radio v-for="item in ${field.enumNameAllUpper}_ENUM" :key="item.value"
                                      :value="item.value">{{ item.label }}
                            </el-radio>
                        </el-radio-group>
                    <#elseif field.formType == 'checkbox'>
                        <el-checkbox-group v-model="state.dataForm.${field.attrName}">
                            <el-checkbox v-for="item in ${field.enumNameAllUpper}_ENUM" :key="item.value"
                                         :label="item.label" :value="item.value">{{ item.label }}
                            </el-checkbox>
                        </el-checkbox-group>
                    <#elseif field.formType == 'date'>
                        <el-date-picker type="date" placeholder="请选择${field.fieldComment!}"
                                        v-model="state.dataForm.${field.attrName}"></el-date-picker>
                    <#elseif field.formType == 'datetime'>
                        <el-date-picker type="datetime" placeholder="请选择${field.fieldComment!}"
                                        v-model="state.dataForm.${field.attrName}"></el-date-picker>
